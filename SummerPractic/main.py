...
@app.post("/panamas/", response_model=schemas.Panama)
def create_panama(panama: schemas.Panama, db: Session = Depends(get_db)):
    db_panama = crud.get_panama_by_url(db, url=panama.url)
